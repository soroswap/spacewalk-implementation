...
export class VaultService {
  ...
  async requestIssuance(uri, amount) {
    console.log(
      ...
      await this.api.api.tx.issue
        ...
        .signAndSend(origin, { nonce }, (submissionResult) => {
          ...
          if (status.isFinalized) {
            ...
            let issueEvents = events.filter((event) => {
              return (
            ...
            if (event.length == 0) {
              reject(new Error("No issue event found", "Issue Request Event"));
            }
      ...
      await this.api.api.tx.redeem
        ...
        .signAndSend(origin, { nonce }, (submissionResult) => {
          ...
          if (status.isFinalized) {
            ...
            if (event.length == 0) {
              reject(
                new Error(`No redeem event found for account ${origin.address}`)
              );
            }
            //we should only find one event corresponding to the issue request
            if (event.length != 1) {
  ...
  handleDispatchError(
    ...
    if (dispatchError?.isModule) {
      ...
      console.log(
        `Extrinsic failed in phase ${phase.toString()} with ${section}.${method}:: ${eventName}`
      );
